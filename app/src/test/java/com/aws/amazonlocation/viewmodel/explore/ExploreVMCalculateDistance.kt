...
class ExploreVMCalculateDistance : BaseTest() {
        ...
        runTest {
                ...
                .`when`(
                    ...
                    when (mode) {
                        ...
                        RouteTravelMode.Scooter.value ->
                            callback.distanceSuccess(
                                Responses.RESPONSE_CALCULATE_DISTANCE_SCOOTER
                            )
            ...
            mExploreVM.mCalculateDistance.test {
                ...
                Assert.assertTrue(
                    ...
                    mExploreVM.mStartLatLng == start
                )
                Assert.assertTrue(
                    TEST_FAILED_DUE_TO_STATE_NOT_LOADING,
                    mExploreVM.mDestinationLatLng == end
                )
                Assert.assertTrue(
                    TEST_FAILED_DUE_TO_STATE_NOT_LOADING,
                ...
                Assert.assertTrue(
                    TEST_FAILED_DUE_TO_INCORRECT_DATA,
                    (result as? HandleResult.Success)?.response?.calculateRouteResult == Responses.RESPONSE_CALCULATE_DISTANCE_SCOOTER
                )
    ...
    @Test
    fun calculateDistanceInternetError() =
        runTest {
            Mockito
                .`when`(
                    mRemoteDataSourceImpl.calculateRoute(

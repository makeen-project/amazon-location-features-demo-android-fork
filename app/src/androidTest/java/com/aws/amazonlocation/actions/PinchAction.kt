...
fun pinchIn(): ViewAction {
    return object : ViewAction {
        ...
        override fun perform(uiController: UiController, view: View) {
            val middlePosition: Point = getCenterPoint(view)
            val startDelta = (view.width * 0.45).toInt() // How far from the center point each finger should start (note: Be sure to have this large enough so that the gesture is recognized!)

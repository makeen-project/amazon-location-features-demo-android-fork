...
import com.aws.amazonlocation.TEST_WORD_11
...
@HiltAndroidTest
class CheckGrabGoButtonTest : BaseTestMainActivity() {
    ...
    fun showGoButtonClickLiveNavigationTest() {
        try {
            ...
            sourceEdt?.perform(click(), replaceText(TEST_WORD_11))
            ...
            val mBottomSheetSearchPlaces: BottomSheetBehavior<ConstraintLayout> =
                BottomSheetBehavior.from(bottomSheetDirectionSearch)
            mBottomSheetSearchPlaces.state = BottomSheetBehavior.STATE_EXPANDED

            Thread.sleep(DELAY_2000)

            val cardBicycleGo =
                mActivityRule.activity.findViewById<MaterialCardView>(R.id.card_bicycle_go)

            Assert.assertTrue(cardBicycleGo.visibility == View.VISIBLE)
        } catch (e: Exception) {
            failTest(203, e)

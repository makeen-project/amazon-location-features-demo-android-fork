...
class ChangeMultipleDepartOptionsTest : BaseTestMainActivity() {
    ...
    fun changeMultipleDepartOptionsTest() {
        try {
            ...
            waitForView(allOf(withId(R.id.cl_drive_loader), isDisplayed()))
            waitForView(allOf(withId(R.id.card_drive_go), isDisplayed()))
            ...
            waitForView(allOf(withId(R.id.tv_depart_options), isDisplayed()))
                ?.check { view, _ ->
                    ...
                    Assert.assertTrue(
                        ...
                        tv?.text?.contains(TEST_WORD_ARRIVE, true) == true
                    )
                }

            waitForView(allOf(withId(R.id.cl_leave_at), isDisplayed()))
                ?.perform(click())
            ...
            waitForView(allOf(withId(R.id.tv_depart_options), isDisplayed()))
                ?.check { view, _ ->
                    ...
                    Assert.assertTrue(
                        TEST_FAILED_DUE_TO_WORD_MISMATCHED,
                        tv?.text?.contains(TEST_WORD_LEAVE, true) == true
                    )
                }
